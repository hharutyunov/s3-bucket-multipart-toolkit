...
Bucket.prototype.copyLargeFile = function(customParams, appendPrefix) {
  var autoAppendPrefix = appendPrefix || false;
  var S3 = this.S3;
  var bucketName = this.bucketName || '';
  var bucketACL = this.bucketACL || '';
  ...
  var uploadCopyPromises = [];
  return new Promise(function (resolve, reject) {
    return createMultipartUploadPromise(params)
      .then(function(response) {
        // console.log('cp => ', response);
        var url = `https://${Bucket}.s3.amazonaws.com/${Key}`;
        copyPartParams.UploadId = response.UploadId
        for(var i = 1; i <= PARTS_COUNT; i++){
          copyPartParams.PartNumber = i;
          copyPartParams.CopySource = copySource;
          uploadCopyPromises.push(uploadPartCopyPromise(copyPartParams));
        }
        return Promise.all(uploadCopyPromises).then(function (tags) {
          var partsForUploadCompletion = tags.map(function(t, tIndex) {
            return  {
              PartNumber: tIndex + 1,
              ETag: t.ETag
            }
          ...
          var completionParams = {
            ...
            MultipartUpload: {
              Parts: partsForUploadCompletion
            }
          };
          completeMultipartUploadPromise(completionParams).then(function (completeResponse) {
            resolve({
              url: url,
